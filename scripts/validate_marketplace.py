...
def _validate_plugin(plugin):
    """Validate a single plugin and return a list of errors."""
    errors = []
...
def validate_marketplace():
    ...
    if "plugins" in marketplace:
        ...
        # Optimization: Parallelize I/O-bound plugin validation tasks using ThreadPoolExecutor
        # to mitigate the performance impact of repeated synchronous file reads and directory
        # traversals, reducing overall execution time linearly with the number of workers (max_workers=10).
        import concurrent.futures

        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(_validate_plugin, plugin) for plugin in plugins]
