...
def _validate_plugin_skills(plugin_dir, name):
    """Validate SKILL.md files within a plugin."""
    errors = []
...
def _validate_plugin(plugin):
    """Validate a single plugin's structure and configuration."""
    errors = []
...
def validate_marketplace():
    ...
    if "plugins" in marketplace:
        ...
        if not plugins:
            ...
            # ⚡ OPTIMIZATION: Parallelize I/O-bound plugin validation tasks
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(_validate_plugin, p) for p in plugins]
