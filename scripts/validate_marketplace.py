...
def validate_plugin(plugin):
    """Validate a single plugin's files."""
    errors = []
...
def validate_marketplace():
    ...
    if "plugins" in marketplace:
        ...
        if not plugins:
            ...
            # Optimization: Parallelize I/O-bound tasks using ThreadPoolExecutor
            # to mitigate performance impact of repeated synchronous file reads.
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                for res in executor.map(validate_plugin, plugins):
                    errors.extend(res)
