...
_VIETNAMESE_DIACRITICS_RE = re.compile(f"[{_VN_BASE}{_VN_BASE.upper()}]")
...
def _check_pair(old: str, new: str) -> list[tuple[str, str, str]]:
    ...
    old_diacritics = _VIETNAMESE_DIACRITICS_RE.findall(old)

    if old_diacritics:
        new_diacritics = _VIETNAMESE_DIACRITICS_RE.findall(new)
