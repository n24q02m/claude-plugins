...
_VIETNAMESE_DIACRITIC_RE = re.compile(f"[{_VN_BASE}{_VN_BASE.upper()}]")
...
def _check_pair(old: str, new: str) -> list[tuple[str, str, str]]:
    ...
    # Performance: Extract diacritics in one pass using C-level regex engine
    old_diacritics = _VIETNAMESE_DIACRITIC_RE.findall(old)

    if old_diacritics:
        new_diacritics = _VIETNAMESE_DIACRITIC_RE.findall(new)
