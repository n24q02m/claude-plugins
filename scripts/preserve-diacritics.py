...
# Optimization: Pre-compile hunk header regex for faster git diff parsing.
...
def _check_pair(old: str, new: str) -> list[tuple[str, str, str]]:
    ...
    # Performance: Use compiled regex findall to extract diacritics instead of
    # iterating over all string characters when disjoint checks fail.
    old_diacritics = _VN_RE.findall(old)

    if old_diacritics:
        new_diacritics = _VN_RE.findall(new)
