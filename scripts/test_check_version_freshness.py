...
class TestCheckVersionFreshness(unittest.TestCase):
    ...
    def test_check_plugin_gemini_fallback(self, mock_urlopen):
        ...
        def mock_open_side_effect(path, *args, **kwargs):
            ...
            m.read.return_value = '{"version": "2.0.0"}'
            return m

        with (
            patch("check_version_freshness.open", side_effect=mock_open_side_effect, create=True),
    ...
    def test_check_plugin_cache(self, mock_urlopen):
        ...
        with patch("check_version_freshness.open", side_effect=FileNotFoundError):
