...
## 2026-08-20 - Hoisted regular expressions in Markdown parsing loop
**Learning:** During profiling of `scripts/verify_tool_parity.py`, replacing `re.match` and `re.fullmatch` string literals with module-level pre-compiled regular expressions within the `declared_names` parsing loop (which runs repeatedly over lines of large markdown files) reduced parse time by roughly 32% (2.69s to 1.82s over 1000 iterations). Because Python's internal regex compilation cache incurs overhead on every call for dynamically executed string patterns, hoisting fixed patterns like `^##\s+(.+?)\s*$` out of tight loops provides measurable performance wins.
**Action:** When parsing large or numerous text files iteratively, always extract static regular expression strings into `re.compile()` objects at the module level before entering the loop to eliminate cache lookup overhead.
