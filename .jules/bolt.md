...
## 2025-02-25 - Extracted static regex compilations and optimized markdown parsing
**Learning:** Compiling regex patterns at the module level rather than calling `re.match()` directly with string patterns in a high-frequency loop (like parsing large markdown tables) provides substantial performance gains by avoiding internal caching overhead. Similarly, replacing `.read().splitlines()` with an iterative line-by-line file reader avoids loading large text blobs into memory and creating large intermediate lists, while matching performance when explicitly stripping newlines.
**Action:** Use module-level `re.compile()` for static regexes in frequently executed loops, and prefer iterative line-by-line readers (`for line in file:`) over `splitlines()` for text processing.
