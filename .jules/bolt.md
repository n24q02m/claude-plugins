...
## 2026-09-03 - Hoisted and cached schema validation regex compilation
**Learning:** In `scripts/test_harness_adapter_schema.py`, the `_validate_json` method was repeatedly invoking `re.search(schema["pattern"], instance)`. Because JSON schemas often define static regex patterns, evaluating these across many fields and nested schemas meant Python's internal regex compilation cache lookup was paid continuously, which adds up in tight validation loops. Decorating a compilation helper `_compile_pattern` with `@functools.lru_cache(maxsize=128)` and storing the compiled `re.Pattern` objects reduces the search overhead by almost 50% compared to repeatedly calling module-level `re.search` with a string pattern.
**Action:** When a method repeatedly matches regular expressions using patterns extracted from stable configurations or schemas (e.g., inside recursive schema validation), wrap the regex compilation in `@functools.lru_cache` to bypass internal compilation and cache lookup overhead, using the bound `.search()` or `.match()` methods directly.
