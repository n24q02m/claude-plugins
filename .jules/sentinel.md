...
## 2024-05-24 - Authorization Header Leakage on HTTPS Downgrade
**Vulnerability:** The `NoAuthRedirectHandler` in `scripts/check_version_freshness.py` correctly stripped the `Authorization` header during cross-origin redirects, but failed to do so when the redirect remained on the same host but downgraded from `https://` to `http://`. This could leak sensitive tokens to network snoopers. Furthermore, the `Authorization` header could bypass the `m.remove_header()` method if it was injected via `unredirected_hdrs`.
**Learning:** HTTPS-to-HTTP downgrades on the same origin are a common blind spot in SSRF/Token-Leak mitigation. When customizing Python's `urllib.request` RedirectHandler, explicitly clearing `unredirected_hdrs` in addition to calling `m.remove_header()` is necessary because the default implementation carries over both.
**Prevention:** Always parse the URL scheme alongside the hostname during redirect checks. If the scheme changes from `https` to `http`, treat it with the same caution as a cross-origin redirect and strip all sensitive headers (from both `headers` and `unredirected_hdrs`).
