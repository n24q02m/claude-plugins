...
## 2026-07-09 - Fix Path Traversal Vulnerability caused by os.path caching
**Vulnerability:** Memoizing `os.path.abspath` or `os.path.realpath` using `@functools.lru_cache` based purely on string values (e.g., `base_dir` in `scripts/utils.py`) introduced a critical path resolution bug. If the current working directory changes, cached relative paths resolve incorrectly, and cached `realpath` values ignore underlying file system modifications like symlink changes, potentially bypassing directory traversal protections.
**Learning:** File system state (such as the current working directory or the resolution of symlinks) is mutable and exists outside the pure function scope of path strings. Caching path resolution functions creates race conditions and incorrect resolutions when the underlying environment changes, breaking security checks like `get_safe_path` that rely on accurate physical path resolution.
**Prevention:** Never use `@functools.lru_cache` or similar caching mechanisms for functions that resolve file paths (like `os.path.abspath` or `os.path.realpath`) unless the cache key explicitly includes all relevant environment state, or the environment is strictly guaranteed to be immutable. For general utilities, avoid caching file system state entirely.
