...
## 2025-06-16 - Reduce stdin Read Limit in MCP Hooks
**Vulnerability:** The centralized `read_mcp_hook_input` function in `plugins/mcp_common.py` read up to 1MB of JSON payload from `sys.stdin`. This could lead to large input exhaustion and Denial-of-Service (DoS) if an attacker submitted extremely large payloads to the hooks.
**Learning:** Stdin parsing and JSON validation for MCP hooks must be hardened against large input exhaustion by using a more restrictive and reasonable payload size limit (e.g., 64KB).
**Prevention:** Stdin parsing and JSON validation for MCP hooks are centralized in `plugins/mcp_common.read_mcp_hook_input()`. This function should read up to 64KB from stdin (hardened from 1MB to mitigate large input exhaustion) and intentionally catches all `Exception` types to ensure a 'fail-closed' security posture.
