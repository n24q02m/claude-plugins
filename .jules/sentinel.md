...
## 2025-07-02 - Fix GraphQL Injection Vulnerability
**Vulnerability:** In `scripts/check_version_freshness.py`, the `_fetch_latest_tags_graphql` function used direct string interpolation to embed the `owner` and `repo` names directly into the GraphQL query payload. If malicious data was introduced via `marketplace.json`, it could manipulate the structure of the GraphQL query, leading to GraphQL injection.
**Learning:** When constructing GraphQL queries (or any database queries) dynamically with user-controlled or external data, always use parameterized variables instead of string interpolation to prevent injection attacks.
**Prevention:** Avoid direct string interpolation of unverified data into the query payload to prevent GraphQL injection vulnerabilities. Parameterize the query by defining variables and passing them via a separate `variables` dictionary payload.
