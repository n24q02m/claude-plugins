...
## 2025-02-28 - Fix HTTPS downgrade credential leak in NoAuthRedirectHandler
**Vulnerability:** The custom redirect handler `NoAuthRedirectHandler` stripped `Authorization` and `Cookie` headers when redirecting across different hostnames to prevent SSRF credential leaks. However, it did not strip these headers when redirecting from an `https://` URL to an `http://` URL on the same hostname. A man-in-the-middle could trigger an HTTP downgrade redirect to intercept the credentials over plain text.
**Learning:** Checking for hostname changes is not sufficient for SSRF mitigation. Redirects that downgrade the connection security (HTTPS to HTTP) also pose a significant risk of credential leakage, even if the destination hostname is the same. It is also important to strip credentials from the `unredirected_hdrs` dictionary on `urllib.request.Request` to ensure headers aren't inadvertently re-added.
**Prevention:** Always check both the hostname and the URL scheme when evaluating redirect safety. Drop sensitive headers if the scheme is downgraded from HTTPS to HTTP. Also ensure to clear sensitive fields from `unredirected_hdrs`.
