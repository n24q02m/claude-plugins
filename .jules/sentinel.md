...
## 2026-07-08 - Fix path validation caching vulnerability in _resolve_base_dir
**Vulnerability:** The `_resolve_base_dir` utility in `scripts/utils.py` memoized `os.path.abspath` and `os.path.realpath` using `@functools.lru_cache` based purely on string values. This caching introduced a critical security bug because cached relative paths become incorrect if the current working directory changes, and cached `realpath` values ignore underlying symlink modifications, leading to potential path traversal bypasses.
**Learning:** Caching path resolutions that depend on dynamic file system states (like CWD or symlink targets) introduces correctness and security vulnerabilities in long-running processes or parallel execution contexts.
**Prevention:** Never use `@functools.lru_cache` on file system operations like `os.path.abspath` or `os.path.realpath` whose outcomes depend on external state beyond the function arguments.
