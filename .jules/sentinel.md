...
## 2024-09-17 - Script CLI Argument Input Bounds
**Vulnerability:** A script parsed CLI inputs using argparse without setting maximum or numeric range bounds. This could allow denial of service (DoS) or resource exhaustion by supplying extremely large numeric bounds via `--timeout`.
**Learning:** Argument bounds and validity checking need to be explicitly enforced after parsing, even for internal scripts, to prevent resource exhaustion vulnerabilities.
**Prevention:** Explicitly enforce input bounds for arguments immediately after `parse_args()`.
