...
> **2026-05-02 Update**: Plugin install (Method 1) now uses pure stdio mode with `NOTION_TOKEN` env var.
...
Marketplace installs across the current stack default to stdio. Container and HTTP paths are explicit self-host overrides, and distribution availability is server-specific.
