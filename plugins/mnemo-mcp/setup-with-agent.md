...
> Optional cloud providers (Jina/Gemini/OpenAI/Cohere) and Google Drive sync are still supported -- set them via env vars (Option 1/2) or configure them through the relay form in HTTP mode (see [setup.md](setup.md), Method 3).
...
remain untouched; Options 2 and 3 build from the current source checkout.
...
## Option 2: Source-built Docker stdio (fallback)
...
Clone the source repository and build the stdio target:
...
For self-hosting HTTP mode, see [setup.md](setup.md), Method 3.
...
## Option 3: Source-built Docker HTTP
...
See [setup.md](setup.md), Method 3, for full instructions on self-hosting the multi-user HTTP mode (per-JWT-sub credential isolation, browser GDrive OAuth, relay password edge auth).
