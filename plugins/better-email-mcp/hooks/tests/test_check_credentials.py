...
class TestCheckCredentials(unittest.TestCase):
    ...
    @patch("sys.stdin", io.StringIO(json.dumps({"tool_name": "list_emails"})))
    @patch("sys.exit", side_effect=SystemExit)
    ...
    def test_main_hint(self, mock_stdout, mock_exit, mock_exists):
        ...
        self.assertIn(
            "better-email-mcp: credentials not yet configured", output["message"]
        )

    @patch.dict(os.environ, {"EMAIL_CREDENTIALS": "creds"}, clear=True)
    @patch("sys.stdin", io.StringIO(json.dumps({"tool_name": "list_emails"})))
    @patch("sys.exit", side_effect=SystemExit)
