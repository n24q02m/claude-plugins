...
class TestCheckCredentials(unittest.TestCase):
    ...
    def test_is_configured_file(self, mock_expanduser, mock_exists):
        ...
        # Built with os.path.join so the separator matches the host OS.
        home_config = os.path.join("/home/user", ".config", "mcp", "config.enc")
        mock_exists.side_effect = lambda p: p == home_config
        self.assertTrue(check_credentials._is_configured())
    ...
    @patch("sys.stdin", io.StringIO(json.dumps({"tool_name": "any_tool__config"})))
    ...
    @patch("sys.stdin", io.StringIO(json.dumps({"tool_name": "any_tool__time"})))
    @patch("sys.exit", side_effect=SystemExit)
    def test_main_exempt_time_tool(self, mock_exit):
        # `time` needs no Google account, so it must not trigger the hint.
        with self.assertRaises(SystemExit):
            check_credentials.main()
        mock_exit.assert_called_with(0)
