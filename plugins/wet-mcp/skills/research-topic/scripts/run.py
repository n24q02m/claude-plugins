...
async def main(argv: list[str] | None = None) -> int:
    ...
    if len(args.query) > 1000:
        parser.error("query must be at most 1000 characters long")
    ...
    if args.synthesis_model and len(args.synthesis_model) > 100:
        parser.error("synthesis_model must be at most 100 characters long")
    if not (100 <= args.token_budget <= 100000):
        parser.error("token_budget must be between 100 and 100000")
