...
class TestCheckCredentials(unittest.TestCase):
    ...
    @patch.dict(os.environ, {"JINA_AI_API_KEY": "sk-123"}, clear=True)
    def test_is_configured_jina(self):
        self.assertTrue(check_credentials._is_configured())

    @patch.dict(os.environ, {"GEMINI_API_KEY": "sk-456"}, clear=True)
    ...
    def test_main_unconfigured_hint(self, mock_stdout, mock_exit, mock_exists):
        ...
        self.assertIn("Note: wet-mcp cloud credentials not configured", output)

    @patch.dict(os.environ, {"OPENAI_API_KEY": "sk-789"}, clear=True)
    @patch('sys.stdin', io.StringIO(json.dumps({"tool_name": "search"})))
    @patch('sys.exit', side_effect=SystemExit)
