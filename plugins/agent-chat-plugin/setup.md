...
Or clone the repository and invoke the stdlib-only entry point directly. Keep `chat.py` and the adjacent `agent_chat/` package together; copying only `chat.py` is not a standalone installation.
...
The PyPI package installs the CLI and Python modules, not harness Skills or lifecycle hooks.
...
Set `AGENT_CHAT_NAME`, `AGENT_CHAT_ROOT`, and optionally `AGENT_CHAT_CHANNELS` in the plugin configuration. These are optional Claude Code lifecycle hooks, not automatic native OMP integration. They post or surface filesystem events and never approve or execute work.
