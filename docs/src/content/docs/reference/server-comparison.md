...
Every repo but one is MIT-licensed. `better-workspace-mcp` is **Apache-2.0**, because it vendors Apache-2.0 code from [gemini-cli-extensions/workspace](https://github.com/gemini-cli-extensions/workspace) and keeps that license for the whole repo.

Forks and self-host welcome either way — no license fees, no telemetry phoning home.
